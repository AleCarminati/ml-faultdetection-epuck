...
bool CRandomWalkBehavior::Action(
    CCI_EPuckProximitySensor::TReadings& tProxReads,
    CCI_RangeAndBearingSensor::TReadings& tRABReads, Real* wheelVelocity,
    std::string id_str) {
  if (CConfiguration::CHANGE_DIRECTION_PROBABILITY_RANDOMWALK >=
      CSourceOfRandomness::m_pcRNG->Uniform(CRange<Real>(0.0, 1.0))) {
    Real fSpeed;

    CRadians angle = CSourceOfRandomness::m_pcRNG->Uniform(
        CRange<CRadians>(-CRadians::PI, CRadians::PI));

    fSpeed =
        angle.GetAbsoluteValue() * CConfiguration::HALF_INTERWHEEL_DISTANCE;
