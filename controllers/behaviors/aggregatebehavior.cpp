...
bool CAggregateBehavior::Action(CCI_EPuckProximitySensor::TReadings& tProxReads,
    ...
    for(size_t i = 0; i <  tRABReads.size(); ++i)
    {
        ...
        {
            m_cAggregationVector += CVector2(tRABReads[i].Range, tRABReads[i].HorizontalBearing);
            robotsinrange++;
        }
    }
    ...
    CSourceOfRandomness::m_pcRNG->Uniform(CRange<CRadians>(-CRadians::PI, CRadians::PI));

    CVector2 m_cHeadingVector =  GetMaxWheelSpeed()  * (1.0f * m_cAggregationVector.Normalize() +
        1.0f * CVector2(1.0f, CSourceOfRandomness::m_pcRNG->Uniform(CRange<CRadians>(-CRadians::PI, CRadians::PI))));
