...
CBehavior::CBehavior(Real maxWheelSpeed):
    m_maxWheelSpeed(maxWheelSpeed){}

CBehavior::~CBehavior(){}

Real CBehavior::GetMaxWheelSpeed(){
...
/* The code of this functions is taken and adapted from Tarapore_PLOSONE_2017, more
 * precisely from the file behavior.cpp. */
void CBehavior::WheelSpeedFromHeadingVector(CVector2& m_cHeadingVector, Real* wheelVelocity){
    ...
    if(cHeadingAngle.GetAbsoluteValue() <= CConfiguration::NO_TURN_THRESHOLD.GetValue())
    {
        /* No Turn, heading angle very small */
        TurningMechanism = CConfiguration::NO_TURN;
    }
    else if(cHeadingAngle.GetAbsoluteValue() > CConfiguration::NO_TURN_THRESHOLD.GetValue() &&
            cHeadingAngle.GetAbsoluteValue() <= CConfiguration::SOFT_TURN_THRESHOLD.GetValue())
    {
        /* Soft Turn, heading angle in between the two cases */
        TurningMechanism = CConfiguration::SOFT_TURN;
    }
    else if(cHeadingAngle.GetAbsoluteValue() > CConfiguration::SOFT_TURN_THRESHOLD.GetValue())
    {
    ...
    if(TurningMechanism == CConfiguration::NO_TURN){
        ...
        Real fSpeedFactor = (CConfiguration::SOFT_TURN_THRESHOLD.GetValue() - cHeadingAngle.GetAbsoluteValue()) /
         ...
         *
         */
        if (fSpeed1 > m_maxWheelSpeed)
            fSpeed1 = m_maxWheelSpeed;

        if (fSpeed2 > m_maxWheelSpeed)
