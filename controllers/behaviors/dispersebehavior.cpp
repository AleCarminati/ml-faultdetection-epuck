...
bool CDisperseBehavior::Action(CCI_EPuckProximitySensor::TReadings& tProxReads,
  ...
  if (m_cDiffusionVector.Angle().GetAbsoluteValue() <
    ...
    if (m_cDiffusionVector.Length() <
        0.05) /* because of noise, we can have very small non-zero sensor
                 readings. but we don't want to respond to them*/
      return false;
  }
  CVector2 m_cHeadingVector =
      -m_cDiffusionVector.Normalize() * GetMaxWheelSpeed();
