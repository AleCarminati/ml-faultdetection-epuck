...
CFlockingBehavior::CFlockingBehavior(Real maxWheelSpeed):
	...
	m_time_counter(0u){
		/* Initialize all the elements of the accumulator matrix and of the displacement 
		 * arrayusing invalid objects.
		 */ 
		DisplacementAccumulator accumulatorMatrix[N_ROBOTS][CONTROL_STEPS_WINDOW];
		memcpy(m_accumulatorMatrix, accumulatorMatrix, sizeof(DisplacementAccumulator) * N_ROBOTS * CONTROL_STEPS_WINDOW);
...
bool CFlockingBehavior::Action(CCI_EPuckProximitySensor::TReadings& tProxReads,
	...
	if(robotsInRangeFlock > 0u){
		...
		/* It wasn't possible to compute the speed of any other robots => the flocking 
		 * behavior is useless.
		 */
		return false;
	}

	/* Find the distance of the neighbors. The flocking behavior has an aggregation component,
	 * in this way the robots can organize themselves in a compact formation.
	 */
	for(int i=0; i<tRABReads.size(); ++i){
		aggregationVector += CVector2(tRABReads[i].Range, tRABReads[i].HorizontalBearing);
		robotsInRangeAggr += 1;	
	...
	/* The maximum range of the RAB is 100cm. This operation applies range normalization to the average 
	 * distance of the neighbors.
	 */
	averageDistanceNeighbors /= 100.0f;

	/* The heading vector is a weighted sum between aggregation behavior's and flocking behavior's vectors.
	 * The weights are based on the neighbors' distance: if they are near, the flocking behavior prevails
	 * because the robots are already in a compact formation; if they are distant the priority is to move
	 * near them.
	 */
	CVector2 m_cHeadingVector =  GetMaxWheelSpeed() * ((1.0f - averageDistanceNeighbors) * flockingVector.Normalize() +
...
void CFlockingBehavior::UpdateDisplacements(CCI_RangeAndBearingSensor::TReadings& tRABReads, 
		...
		float* lastWheelVel, std::string id_str)
{
	computeDisplacementsMatrix(tRABReads, tDifReads, lastWheelVel, 
		&m_accumulatorMatrix[0][0], m_displacements,CONTROL_STEPS_WINDOW,
		m_time_counter, id_str);

	/* The time counter gets updated at the end of the function because in the first
	 * control step the time counter is equal to 0, that is the first index of the 
	 * array.
	 */
	m_time_counter++;
