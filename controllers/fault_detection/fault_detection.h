...
public:
    ...
    */
   void GetAndCorrectProxReadings(CCI_EPuckProximitySensor::TReadings& sensorsReadings);

   /*
    * This function gets the RAB's readings.
    * It corrects them if the robot is faulty. 
    */
   void GetAndCorrectRABReadings(CCI_RangeAndBearingSensor::TReadings& sensorsReadings);
    ...
    */  
   void AvoidCollisions(CCI_EPuckProximitySensor::TReadings& tProxReads, Real* wheelVelocity);
    ...
    */ 
   void ObserveNeighborsBoolean(CCI_RangeAndBearingSensor::TReadings& tRABReads, 
    ...
    * neighbors, based on sensor readings.
    */
   void ObserveNeighborsNumerical(CCI_RangeAndBearingSensor::TReadings& tRABReads, 
      const CCI_DifferentialSteeringSensor::SReading& tDifReads,
      CCI_EPuckProximitySensor::TReadings& tProxReads);

   /* This function updates the fault probabilities of all the robots in the swarm. */
...
private:
   ...
   /* This vector is used ONLY for testing purposes. It does not affect the fault detection
    * algorithm.
    */
   ...
   /* Contains the string that defines the type of behavior that the swarm follows. */
   std::string m_behavior_str;

   /* Vector that contains the classes that compute the behavior of the robot.
    * shared_ptr gives the possibility to maintain the object outside the Init function 
    * and to use polymorphism. Normal pointers would have lost the object outside the
    * Init function, causing segmentation fault.
    * We use the "shared" version because in the case of flocking behavior there will be
    * another variable that points to that behavior and that is used to update the 
    * displacements.
    */
    ...
    */ 
   DisplacementAccumulator m_displacementsMatrix[N_ROBOTS][TIME_WINDOW_OBSERVATION];
   ...
   /* Defines the number of consecutive control steps in which the robot is colliding. */
   unsigned int m_consecutive_collisions; 

   /* Defines if the robot is a beacon. It is true only when the selected behavior is 
    * homing and the robot has the ID equal to BEACON_ID.
    */
   bool m_beacon;

   /* Maintains the highest proximity sensor's value in the last control step.
    * It is used to check if the robot could have collided during the last control step.
    */
   Real m_nearestProx;
    ...
    */ 
   DisplacementAccumulator m_displacementsMatrixBayes[N_ROBOTS][TIME_WINDOW_BAYES];
