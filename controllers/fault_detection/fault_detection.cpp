...
#define safe_xgboost(call) {  \
  int err = (call); \
  if (err != 0) { \
    THROW_ARGOSEXCEPTION(std::string(__FILE__) + ":" + std::to_string(__LINE__) + \
...
void CEPuckFaultDetection::Init(TConfigurationNode& t_node) {
   ...
   /* The order of the behaviors is taken from Tarapore_PLOSONE_2017, more precisely from epuck_hom_swarm.cpp. */
   if(m_behavior_str.compare(CConfiguration::AGGREGATE)==0){
      m_behavior = std::vector<std::shared_ptr<CBehavior>>();
      m_behavior.push_back(std::shared_ptr<CBehavior>(new CDisperseBehavior(m_fWheelVelocity)));
      m_behavior.push_back(std::shared_ptr<CBehavior>(new CAggregateBehavior(m_fWheelVelocity)));
      m_behavior.push_back(std::shared_ptr<CBehavior>(new CRandomWalkBehavior(m_fWheelVelocity)));
      ...
      if(std::stoi(id_str) != CConfiguration::BEACON_ID){
         m_behavior.push_back(std::shared_ptr<CBehavior>(new CDisperseBehavior(m_fWheelVelocity)));
         m_behavior.push_back(std::shared_ptr<CBehavior>(new CHomingBehavior(m_fWheelVelocity)));
         m_behavior.push_back(std::shared_ptr<CBehavior>(new CRandomWalkBehavior(m_fWheelVelocity)));
      }else{
         m_beacon = true;
      }
   }else{
      THROW_ARGOSEXCEPTION("Wrong value for parameter 'behavior' in .argos file.");
   }
   ...
   if(CConfiguration::BOOLEAN_OBSERVATIONS){
      ...
      memcpy(m_booleanObservations, observations, sizeof(BooleanObservation)
         *N_ROBOTS*N_OBSERVATIONS);
      ...
      memcpy(m_numericalObservations, observations, sizeof(NumericalObservation)
         *N_ROBOTS*N_OBSERVATIONS);
      for(int i=0; i<N_ROBOTS; ++i){
         for(int j=0; j<N_OBSERVATIONS; ++j){
   ...
   /* Allocate the memory needed to store the fault probabilities of the other robots.
    * Initialize every cell with a value that represents that the classification is 
    * invalid. */
   ...
   if(m_training){
      for(int i=0; i<N_ROBOTS; ++i){
         if(i==CConfiguration::ID_FAULTY_ROBOT && m_faultType.compare(CConfiguration::FAULT_NONE)!=0){
   ...
   DisplacementAccumulator displacementsMatrix[N_ROBOTS][TIME_WINDOW_OBSERVATION];
   memcpy(m_displacementsMatrix, displacementsMatrix, sizeof(DisplacementAccumulator)*N_ROBOTS*TIME_WINDOW_OBSERVATION);
   for(int i=0; i<N_ROBOTS; ++i){
   ...
   if(CConfiguration::BOOLEAN_OBSERVATIONS){
      ...
      memcpy(m_neighborsMatrix, neighborsMatrix, sizeof(NeighborAccumulator)*N_ROBOTS*TIME_WINDOW_OBSERVATION);
      for(int i=0; i<N_ROBOTS; ++i){
         for(int j=0; j<TIME_WINDOW_OBSERVATION; ++j){
   ...
   if(CConfiguration::BOOLEAN_OBSERVATIONS){
      ...
      memcpy(m_irrespPriorAlpha, irrespPriorAlpha, sizeof(unsigned int)*N_ROBOTS);
      for(int i=0; i<N_ROBOTS; ++i){
      ...
      memcpy(m_noSensPriorAlpha, noSensPriorAlpha, sizeof(unsigned int)*N_ROBOTS);
      for(int i=0; i<N_ROBOTS; ++i){
      ...
      DisplacementAccumulator displacementsMatrixBayes[N_ROBOTS][TIME_WINDOW_BAYES];
   ...
   /* Initialize the vector that contains the speed of the wheels in the last step. */
...
void CEPuckFaultDetection::ControlStep() {
   ...
   const CCI_DifferentialSteeringSensor::SReading tDifReads = m_pcWheelsSens->GetReading();
   ...
   if(!m_training){
       ...
       */ 
      if(m_timeCounter%(COMPARISON_CONTROL_STEPS+OBSERVE_CONTROL_STEPS)==0){
         WriteToCSVTesting();
      ...
      }else if(m_timeCounter%(COMPARISON_CONTROL_STEPS+OBSERVE_CONTROL_STEPS)<=
   ...
   if(id_int==19){
      if(m_timeCounter >= TIME_WINDOW_OBSERVATION){
         output = std::to_string(CVector2(m_lastPosVector[0].Position.GetX()*100-m_lastPosVector[TIME_WINDOW_OBSERVATION-1].Position.GetX()*100, m_lastPosVector[0].Position.GetY()*100-m_lastPosVector[TIME_WINDOW_OBSERVATION-1].Position.GetY()*100).Length());
      }else{
         output = "";
      }
      output+="\n";
   }else{
      if(m_numericalObservations[19][N_OBSERVATIONS-1].validity && m_numericalObservations[19][N_OBSERVATIONS-1].displacement!=CConfiguration::INVALID_DISPLACEMENT){
         output = std::to_string(m_numericalObservations[19][N_OBSERVATIONS-1].displacement);
      }else{
         output="";
      }
      output+=";";
    ...
    * They are initialized with the velocities in the last step because the behavior 
    * class could decide to not modify the array.
    */ 
   Real wheelVelocity[2] = {0.0f, 0.0f};

   if(m_flockingBehavior && !m_firstStep){
      m_flockingBehavior->UpdateDisplacements(tRABReads, tDifReads, 
         m_lastWheelVel, CCI_Controller::GetId());
   }

   bool stop = false;
   for(std::vector<std::shared_ptr<CBehavior>>::iterator i = m_behavior.begin(); i!= m_behavior.end(); i++){
      if(!stop){
...
void CEPuckFaultDetection::GetAndCorrectProxReadings(CCI_EPuckProximitySensor::TReadings& correctedReadings){
   std::string id_str = CCI_Controller::GetId();
   id_str.erase(0,2);
   UInt8 id_int = std::stoi(id_str);

   const CCI_EPuckProximitySensor::TReadings& sensorsReadings = m_pcProximity->GetReadings();
   ...
   if(id_int==CConfiguration::ID_FAULTY_ROBOT){
      if(m_faultType.compare(CConfiguration::FAULT_PMIN)==0){
         for(int i=0; i<sensorsReadings.size();++i){
            ...
            if(i < sensorsReadings.size() / 4 || i >= sensorsReadings.size() * 3 / 4){
         ...
         for(int i=0; i<sensorsReadings.size();++i){
            ...
            // Modify only the frontal readings.
            if(i < sensorsReadings.size() / 4 || i >= sensorsReadings.size() * 3 / 4){
               correctedReadings[i].Value = CSourceOfRandomness::m_pcRNG->Uniform(range);
            }else{
               correctedReadings[i].Value = sensorsReadings[i].Value;
            }
         }
...
void CEPuckFaultDetection::GetAndCorrectRABReadings(CCI_RangeAndBearingSensor::TReadings& correctedReadings){
   /* Detect the id of the robot. */
   std::string id_str = CCI_Controller::GetId();
   id_str.erase(0,2);
   UInt8 id_int = std::stoi(id_str);

   const CCI_RangeAndBearingSensor::TReadings& sensorsReadings = m_pcRABS->GetReadings();

   if(id_int==CConfiguration::ID_FAULTY_ROBOT && m_faultType.compare(CConfiguration::FAULT_ROFS)==0){
      ...
      for(int i=0; i<sensorsReadings.size();++i){
         ...
         correctedReadings[i].Range = sensorsReadings[i].Range + CSourceOfRandomness::m_pcRNG->Uniform(poolRange);
         correctedReadings[i].HorizontalBearing = sensorsReadings[i].HorizontalBearing + CSourceOfRandomness::m_pcRNG->Uniform(poolBearing);
         correctedReadings[i].VerticalBearing.SetValue(sensorsReadings[i].VerticalBearing.GetValue());
      }
   }else{
      correctedReadings = sensorsReadings;
   }
...
void CEPuckFaultDetection::AvoidCollisions(CCI_EPuckProximitySensor::TReadings& tProxReads, 
   Real* wheelVelocity)
{
   if(tProxReads[0].Value > 0.4f ||
         tProxReads[1].Value > 0.4f ||
         tProxReads[2].Value > 0.4f ||
         tProxReads[3].Value > 0.4f ||
         tProxReads[4].Value > 0.4f ||
         tProxReads[5].Value > 0.4f ||
         tProxReads[6].Value > 0.4f ||
         tProxReads[7].Value > 0.4f){
   ...
   for(size_t i=0; i<tRABReads.size(); ++i){
      ...
      while(!stop && index <= 25+N_ROBOTS*7){
         ...
         if(!stop && observationAccumulator[id].ownObservation){
            ...
            for(int j=0;j<6;++j){
               observationAccumulator[id].featuresAccumulator[j] += 
                  message[index];
               index++;
            }   
         }
...
void CEPuckFaultDetection::ObserveNeighborsBoolean(
   ...
   if(m_firstStep){
      ...
      computeNeighborsMatrix(tRABReads, &m_neighborsMatrix[0][0], 
         observedNeighborsAccumulators, TIME_WINDOW_OBSERVATION, m_timeCounter,
      ...
      if(!collision){
         computeDisplacementsMatrix(tRABReads, tDifReads, m_lastWheelVel,
            &m_displacementsMatrix[0][0], observedDisplacements,
            TIME_WINDOW_OBSERVATION, m_timeCounter, CCI_Controller::GetId());
         computeDisplacementsMatrix(tRABReads, tDifReads, m_lastWheelVel,
            &m_displacementsMatrixBayes[0][0], observedDisplacementsBayes,
            TIME_WINDOW_BAYES, m_timeCounter, CCI_Controller::GetId());
      }else{
      ...
      for(int i = 0; i<N_ROBOTS; ++i){
         ...
         if(numberOfFeatures[i]==2 && 
            ...
            CConfiguration::INVALID_DISPLACEMENT_VECTOR)
         {
            if(observedDisplacements[i].Length() > 
               0.15 * TIME_WINDOW_OBSERVATION * CConfiguration::TIME_STEP * 
               m_fWheelVelocity)
            {
      ...
      for(size_t i=0;i<tRABReads.size();++i){
            ...
            m_prevAngularSpeed[id] != CConfiguration::INVALID_ANGULAR_SPEED)
         {
            ...
            float currentAngularSpeed = (-observedVelLeftWheel+
               observedVelRightWheel)/ CConfiguration::INTERWHEEL_DISTANCE *
            ...
            if(normalizedAngularAcceleration * 
               ...
               (TIME_WINDOW_BAYES * CConfiguration::TIME_STEP * m_fWheelVelocity) > 0.1){
               m = true;
            }else{
               m = false;
            }
            for(int j=0; j<tRABReads.size() && !neighborsInRange;++j){
               ...
               {
                  observedNeighborDistance = sqrt(pow(tRABReads[i].Range, 2) + 
                     pow(tRABReads[j].Range, 2) - 2 * tRABReads[i].Range *
                     tRABReads[j].Range * Cos(tRABReads[i].HorizontalBearing - 
            ...
            if(((float)m_irrespPriorAlpha[id]/(m_irrespPriorAlpha[id] + 
               m_irrespPriorBeta[id])) > 0.05){
               lastObservations[id].features[3] = true;
            }else{
               lastObservations[id].features[3] = false;
            }
            if(((float)m_sensPriorAlpha[id]/(m_sensPriorAlpha[id] + 
               m_sensPriorBeta[id])) > 0.05){
               lastObservations[id].features[4] = true;
            }else{
               lastObservations[id].features[4] = false;
            }
            if(((float)m_noSensPriorAlpha[id]/(m_noSensPriorAlpha[id] + 
               m_noSensPriorBeta[id])) > 0.05){
      ...
      if(CConfiguration::ENDOGENOUS_OBSERVATION && !m_beacon){
         ...
         float prevAngularSpeed = (-m_lastWheelVel[0]+m_lastWheelVel[1]) / 
            CConfiguration::INTERWHEEL_DISTANCE * CConfiguration::TIME_STEP;
         float currentAngularSpeed = (-tDifReads.VelocityLeftWheel+
            tDifReads.VelocityRightWheel)/ CConfiguration::INTERWHEEL_DISTANCE *
            CConfiguration::TIME_STEP;
         float normalizedAngularAcceleration = 
            fabsf(currentAngularSpeed-prevAngularSpeed)/ maxAngularAcceleration;
         if(normalizedAngularAcceleration * 
         ...
         /* Assign the values of the last 3 features.*/
         if((m_irrespPriorAlpha[id_int]/(m_irrespPriorAlpha[id_int] + 
            m_irrespPriorBeta[id_int])) > 0.05){
            lastObservations[id_int].features[3] = true;
         }else{
            lastObservations[id_int].features[3] = false;
         }
         if((m_sensPriorAlpha[id_int]/(m_sensPriorAlpha[id_int] + 
            m_sensPriorBeta[id_int])) > 0.05){
            lastObservations[id_int].features[4] = true;
         }else{
            lastObservations[id_int].features[4] = false;
         }
         if((m_noSensPriorAlpha[id_int]/(m_noSensPriorAlpha[id_int] + 
            m_noSensPriorBeta[id_int])) > 0.05){
      ...
      for(size_t i=0; i<tRABReads.size();++i){
         ...
         message >> beaconByte;
         if(beaconByte!=CConfiguration::BEACON_MESSAGE){
            m_prevAngularSpeed[id] = (-observedVelLeftWheel+
               observedVelRightWheel)/ CConfiguration::INTERWHEEL_DISTANCE *
               CConfiguration::TIME_STEP;
            observed[id] = true;
         }
...
void CEPuckFaultDetection::ObserveNeighborsNumerical(
   ...
   UInt8 beaconByte;
   CVector2 observedDisplacements[N_ROBOTS];
   /* This array controls if a robot has been observed in this control step. Initalized to all false. */
   ...
   /* During the first step, each robot sends a dummy message to connect to its peers. 
    * This message must not be saved by the receivers.
    */
   if(m_firstStep){
      m_firstStep = false;
   }else{
      ...
      if(!collision){
         ...
         /* If the robot probabily collided in the last control step, make all the displacement measurements invalid. */
         for(int i = 0; i<N_ROBOTS; ++i){
            for(int j=0; j<TIME_WINDOW_OBSERVATION; ++j){
               m_displacementsMatrix[i][j].validity = false;
            }            
      ...
      for(size_t i=0; i<tRABReads.size(); ++i){ 
         ...
         if(beaconByte != CConfiguration::BEACON_MESSAGE){
            if(observedDisplacements[observedId] != (CConfiguration::INVALID_DISPLACEMENT_VECTOR)){
            ...
            float observedMinNeighborDistance = CConfiguration::INVALID_MIN_NEIGHBOR_DISTANCE;
            ...
            for(int j = 0; j < tRABReads.size(); ++j){
               if(j!=i && tRABReads[j].Data[CConfiguration::BEACON_BYTE_INDEX] == CConfiguration::NOT_BEACON_MESSAGE){
                  observedNeighborDistance = sqrt(pow(tRABReads[i].Range, 2) + pow(tRABReads[j].Range, 2) - 2 * tRABReads[i].Range *
                     tRABReads[j].Range * Cos(tRABReads[i].HorizontalBearing - tRABReads[j].HorizontalBearing));
            ...
            if(observedNumNeighbors != 0){
               observedAverageNeighborDistance = observedSumNeighborDistance/observedNumNeighbors;
            }

            NumericalObservation lastObservation = {observedVelLeftWheel, 
               observedVelRightWheel, observedDisplacement,
               observedMinNeighborDistance, observedAverageNeighborDistance, 
               true};

            pushNumericalObservation(m_numericalObservations[observedId], lastObservation);
      ...
      if(CConfiguration::ENDOGENOUS_OBSERVATION && !m_beacon){
         ...
         if(observedDisplacements[id_int]!=CConfiguration::INVALID_DISPLACEMENT_VECTOR){
         ...
         NumericalObservation ownObservation = {velLeftWheel, velRightWheel, 
            displacement, minNeighborDistance, avgNeighborDistance, true};
      ...
      /* Add dummy data for the robots that were not observed in this control step. */
      for(int j=0; j<N_ROBOTS; ++j){
         if(!observedRobot[j]){
            /* Add an empty observation for that robot. */
            pushNumericalObservation(m_numericalObservations[j], {0, 0, 0, 0, 0, false});
         }
      }
   }
}
   ...
   for(int i=0; i<N_ROBOTS;++i){
      ...
      if(completeObservation){
         ...
         if(!CConfiguration::BOOLEAN_OBSERVATIONS){
            for(int j=0; j<N_OBSERVATIONS;++j){
               if(m_behavior_str.compare(CConfiguration::AGGREGATE)==0){
                  inputMatrixNum [j*5+0] = (m_numericalObservations[i][j].velLeftWheel - 0.78743926)/4.09533154;
                  inputMatrixNum [j*5+1] = (m_numericalObservations[i][j].velRightWheel - 0.78743926)/4.09533154;
                  inputMatrixNum [j*5+2] = (m_numericalObservations[i][j].displacement - 5.92557105)/8.1276298;
                  inputMatrixNum [j*5+3] = (m_numericalObservations[i][j].minNeighborDistance - 13.38839379)/13.57544033;
                  inputMatrixNum [j*5+4] = (m_numericalObservations[i][j].averageNeighborDistance - 22.64306708)/15.97433554;
                  /*observations = observations + std::to_string(inputMatrixNum [j*5+0])+";";
                  observations = observations + std::to_string(inputMatrixNum [j*5+1])+";";
                  observations = observations + std::to_string(inputMatrixNum [j*5+2])+";";
                  observations = observations + std::to_string(inputMatrixNum [j*5+3])+";";
                  observations = observations + std::to_string(inputMatrixNum [j*5+4])+";";*/
               }else if(m_behavior_str.compare(CConfiguration::DISPERSE)==0){
                  inputMatrixNum [j*5+0] = (m_numericalObservations[i][j].velLeftWheel - 4.25749918)/2.10906098;
                  inputMatrixNum [j*5+1] = (m_numericalObservations[i][j].velRightWheel - 4.25749918)/2.10906098;
                  inputMatrixNum [j*5+2] = (m_numericalObservations[i][j].displacement - 38.29590202)/16.90179461;
                  inputMatrixNum [j*5+3] = (m_numericalObservations[i][j].minNeighborDistance - 34.41989348)/20.13268943;
                  inputMatrixNum [j*5+4] = (m_numericalObservations[i][j].averageNeighborDistance - 70.61601984)/22.21833864;
               }else if(m_behavior_str.compare(CConfiguration::FLOCKING)==0){
                  inputMatrixNum [j*5+0] = (m_numericalObservations[i][j].velLeftWheel - 2.40174834)/3.09620344;
                  inputMatrixNum [j*5+1] = (m_numericalObservations[i][j].velRightWheel - 2.40174834)/3.09620344;
                  inputMatrixNum [j*5+2] = (m_numericalObservations[i][j].displacement - 20.99495627)/10.19071104;
                  inputMatrixNum [j*5+3] = (m_numericalObservations[i][j].minNeighborDistance - 14.17819407)/13.63555213;
                  inputMatrixNum [j*5+4] = (m_numericalObservations[i][j].averageNeighborDistance - 26.64679617)/16.36465572;
               }else if(m_behavior_str.compare(CConfiguration::HOMING)==0){
                  inputMatrixNum [j*5+0] = (m_numericalObservations[i][j].velLeftWheel - 3.09571524)/2.97165248;
                  inputMatrixNum [j*5+1] = (m_numericalObservations[i][j].velRightWheel - 3.09571524)/2.97165248;
                  inputMatrixNum [j*5+2] = (m_numericalObservations[i][j].displacement - 24.48300935)/15.68166334;
                  inputMatrixNum [j*5+3] = (m_numericalObservations[i][j].minNeighborDistance - 22.26954653)/19.82136902;
                  inputMatrixNum [j*5+4] = (m_numericalObservations[i][j].averageNeighborDistance - 45.91643746)/25.53002059;
               }else{
        ...
        -0.14584664, -0.14949432,  0.67208438,  0.34113341,  0.12415187         
         ...
         if(pModule != NULL){
            ...
            if(pFunc && PyCallable_Check(pFunc)){
               ...
               if(pValue != NULL){
                  ...
                  PyErr_Print();
                  unsetenv("PYTHONPATH");
                  THROW_ARGOSEXCEPTION("Python call: the call to the function returned NULL value.");
               }
            }else{
               if(PyErr_Occurred()){
                  PyErr_Print();
               ...
               Py_DECREF(pModule);
               unsetenv("PYTHONPATH");
               THROW_ARGOSEXCEPTION("Python call: can't find the function inside the file.");
            }
         }else{
            PyErr_Print();
            unsetenv("PYTHONPATH");
         ...
         if(probability >=0.0f && probability <=1.0f){
            ...
            THROW_ARGOSEXCEPTION("The returned probability is outside range [0,1].");
         }
      }
   }
}
...
void CEPuckFaultDetection::WriteToCSVTraining(){
   ...
   if(CConfiguration::BOOLEAN_OBSERVATIONS){
      filename = "./data/binary_training/" + m_behavior_str+ "/" + m_behavior_str + "_" +
         m_faultType + "_" +std::to_string(random_seed) + ".csv";
   }else{
      filename = "./data/numerical_training/" + m_behavior_str+ "/" + m_behavior_str + "_" +
         m_faultType + "_" + std::to_string(random_seed) + ".csv";
   }
   ...
   if(CConfiguration::BOOLEAN_OBSERVATIONS){
      ...
      for(int i = 0; i < N_ROBOTS; ++i){
         ...
         std::string datapoint= std::to_string(CSourceOfRandomness::m_pcRNG->GetSeed())+";" + 
         ...
         if(!stop){
            ...
            if(!m_localCoalitionList[id_observed].validity &&
               std::find(m_votesAccumulator[id_observed].idVotersList.begin(),
                m_votesAccumulator[id_observed].idVotersList.end(), id_observer)
                 == m_votesAccumulator[id_observed].idVotersList.end())
            {
               m_votesAccumulator[id_observed].idVotersList.
                  push_back(id_observer);
               if(vote){
