...
/* The number of observations related to a specific node that a robot keeps in memory. */ 
#define N_OBSERVATIONS 10
/* The number of robots in the swarm. */
#define N_ROBOTS 20
/* The number of control steps necessary to measure the displacement of an observed robot. */
...
/* This class reads from a configuration file all the constants useful to run the program. */
...
public:
	...
	/* The maximum force that can be applied to the robot to change its speed. Represented in Newton. */
	static float MAX_FORCE; 
	/* The mass of the robot. Represented in kg. */
	static float MASS;
	/* The acceleration that the robot applies when it needs to change its speed. Represented in cm/s². */
	static float ACCELERATION; 
	/* The time elapsed between two different control steps. Represented in s. */
 	static float TIME_STEP;
 	/* The time elapsed between two different time steps of the physical engine. Represented in s. */
	...
	/* OBSERVATION */
	/* The value given to the observed displacement when it was not possible to compute it. */
	static int INVALID_DISPLACEMENT;
	/* The value given to the minimum distance of the neighbor when no neighbors are seen. 
	 * It is higher than any possible value that the variable can assume. */ 
	static float INVALID_MIN_NEIGHBOR_DISTANCE;
	/* The value given to the fault probability when it was not possible to compute it. */
	...
	/* The value of the thresholds that define when the robot must go straight, make a soft turn or 
	 * an hard turn.
	 * They are taken from the code of Tarapore_PLOSONE_2017, more precisely from the 
	 * file epuck_hom_swarm.h.
	 */ 
	static CRadians NO_TURN_THRESHOLD; 
	static CRadians SOFT_TURN_THRESHOLD;
	/* The value of the integers that represents the type of turn that the robot must do. */
	static int NO_TURN;
	...
	/* A dummy vector that is used when it was not possible to compute the displacement. */
	 ...
	 * This value is taken from Tarapore_PLOSONE_2017, more precisely from epuck_hom_swarm.cpp.
	 */
	static float CHANGE_DIRECTION_PROBABILITY_RANDOMWALK;
	/* The bytes to differentiate the messages that come from a beacon and the ones that come from a normal robot. */ 
	...
	/* The index of the byte that identifies it the sender is a beacon or not. It is computed with the assumption
	 * that the first position has index equal to 0.
	 */ 
	static unsigned int BEACON_BYTE_INDEX; 

	/* This function reads the configuration file and initializes all the variables of the class.
	 * It contains a simple parser to read the .ini files, which supports sections and single-line comments. 
	 * that start with '#'.
	 */
	static void ReadConfigurationFile();

	/* This function prints all the variables in configuration and their respective values. 
	 * It can be used to check if the configuration file has been read correctly.
	 */
...
struct NumericalObservation{
   ...
   /* 
    * This method transforms the data structure in a string that can be inserted into a .csv file.
    */
   std::string toCsvString(){
      std::string csvString;
      if(validity){
         csvString =  std::to_string(velLeftWheel)+";"+std::to_string(velRightWheel)+";";
         /* In order to compute the distance traveled, two consecutive observations are needed.
          * The first of the two observations will have the value of the distance traveled equal 
          * to a dummy value, because it was impossible to compute it.
          */  
         ...
         if(minNeighborDistance != CConfiguration::INVALID_MIN_NEIGHBOR_DISTANCE){
            csvString +=  std::to_string(minNeighborDistance)+";"+std::to_string(averageNeighborDistance)+";";
         }else{
            csvString += ";;";
         }
...
/* Auxiliary structure that, given time t, stores the observed robot's position at time t,
 * the future movements of the observer robot w.r.t. the observer robot's reference frame 
 * at time t. Its data will be used at time t + time_window to compute the displacement of 
 * the observed robot during time_window.
 * Time t and the id of the observed robot are not explicitly represented in this structure
 * because their management is left to the class that uses the structure.  
 */
struct DisplacementAccumulator
{
	...
	/* Two accumulators. They store, respectively, the translation and the rotation of the observer
	 * from time t to the current control step, both w.r.t. the observer robot's reference frame at 
	 * time t. 
	 */
	CVector2 observerTranslation;
	CRadians observerRotation;

	/* This variable defines if the accumulator is valid and can be used to compute a displacement.
	 * It will be true if the initial position of the observed robot at time t was observed, otherwise
	 * it will be false.
	 */
	bool validity;
};
...
/* This function computes the displacements of the observed robots from a matrix of DisplacementAccumulator.
 * The matrix must have N_ROBOTS rows, one for each robot in the swarm, and a number of columns equal
 * to the time window used to compute the displacement. 
 * The results of the displacement computations will be inserted in an array of length equal to the number
 * of robots in the swarm. The measure of the time must be provided by the caller function.
 * 
 * Note about the implementation: the second dimension of the matrix (the number of columns) is not 
 * known at compile time because it depends on the caller function. Thus, selection of the row using
 * square brackets is impossible because the function is not able to determine the offset between the 
 * address of a row and the address of the subsequent row, that is equal to the size of the struct multiplied
 * by the number of rows. 
 * This problem has been resolved substituting the square brackets with the correct arithmetic function
 * applied to the pointer, which uses TIME_WINDOW and can be evaluated at runtime.
 */
...
/* This function computes the transition and the rotation of the robot in the last 
 * control step. The variable initialOmega represents the angle between the heading 
 * of the robot at the start of the control step and the vertical axis of the reference
 * frame.
 * The velocities are considered to be in cm/s, the distances in cm and the angles in radians.
 */
...
/* This function takes a string and replaces all the characters equal to characterToFind with
 * the character characterToReplace.
 * It returns the number of substitutions done.
 */
