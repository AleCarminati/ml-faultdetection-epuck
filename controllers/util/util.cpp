...
/* Initialization of the source of randomness. Its seed will be based on the seed 
 * selected in the .argos file.
 */
	...
	while(!fstrm.eof()){
		...
		if(!line.empty() && line[0]!='#' && line[0]!='['){
			...
			if(equalIndex == std::string::npos){
				THROW_ARGOSEXCEPTION("Missing '=' in line " + std::to_string(lineNumber) +".");
			}
			...
			if(leftSide.compare("INTERWHEEL_DISTANCE")==0){
				...
				float denominator = std::stof(rightSide.substr(divisionIndex+1, std::string::npos));
				...
				float y = std::stof(rightSide.substr(commaIndex+1, std::string::npos));
				INVALID_DISPLACEMENT_VECTOR = CVector2(x,y);
			}else if(leftSide.compare("PROXIMITY_SENSOR_THRESHOLD_DISPERSION")==0){
				PROXIMITY_SENSOR_THRESHOLD_DISPERSION = std::stof(rightSide);
			}else if(leftSide.compare("GO_STRAIGHT_ANGLE_THRESHOLD_DISPERSION")==0){
				int divisionIndex = rightSide.find("/");
				if(divisionIndex == std::string::npos){
					THROW_ARGOSEXCEPTION("Missing '/' in line " + std::to_string(lineNumber) +".");
				}
				float numerator = std::stof(rightSide.substr(0, divisionIndex));
				float denominator = std::stof(rightSide.substr(divisionIndex+1, std::string::npos));
				GO_STRAIGHT_ANGLE_THRESHOLD_DISPERSION = CRadians::PI * numerator / denominator;
			}else if(leftSide.compare("CHANGE_DIRECTION_PROBABILITY_RANDOMWALK")==0){
				...
				THROW_ARGOSEXCEPTION("Unable to recognize the variable on the left side in line " +
						std::to_string(lineNumber) +".");
			}
		}
	}

	/* Set the constants that can be derived from the ones read in the config file. */
	HALF_INTERWHEEL_DISTANCE = INTERWHEEL_DISTANCE / 2.0f;
	ACCELERATION = MAX_FORCE * 100.0f / MASS;

	/* To check if the function parsed correctly the configuration file, uncomment the 
	 * next line.
	 */ 
	//PrintConfiguration();
...
void CConfiguration::PrintConfiguration(){
	...
	output += "[GENERAL]\n";
	output += "INTERWHEEL_DISTANCE = " + std::to_string(CConfiguration::INTERWHEEL_DISTANCE) + "\n";
 	output += "HALF_INTERWHEEL_DISTANCE = " + std::to_string(CConfiguration::HALF_INTERWHEEL_DISTANCE) + "\n";
 	output += "MAX_FORCE = " + std::to_string(CConfiguration::MAX_FORCE) + "\n";
	output += "MASS = " + std::to_string(CConfiguration::MASS) + "\n";
	output += "ACCELERATION = " + std::to_string(CConfiguration::ACCELERATION) + "\n";
	output += "TIME_STEP = " + std::to_string(CConfiguration::TIME_STEP) + "\n";
	output += "TIME_STEP_PHYSICS = " + std::to_string(CConfiguration::TIME_STEP_PHYSICS) + "\n";
	...
	output += "ID_FAULTY_ROBOT = " + std::to_string(CConfiguration::ID_FAULTY_ROBOT) + "\n";

	output += "\n[OBSERVATION]\n";
	output += "INVALID_DISPLACEMENT = " + std::to_string(CConfiguration::INVALID_DISPLACEMENT) + "\n";
	output += "INVALID_MIN_NEIGHBOR_DISTANCE = " + std::to_string(CConfiguration::INVALID_MIN_NEIGHBOR_DISTANCE) + "\n";
	output += "INVALID_FAULT_PROBABILITY = " + std::to_string(CConfiguration::INVALID_FAULT_PROBABILITY) + "\n";
	output += "INVALID_ANGULAR_SPEED = " + std::to_string(CConfiguration::INVALID_ANGULAR_SPEED) + "\n";
	output += "COLLISION_CONSERVATIVE = " + std::to_string(CConfiguration::COLLISION_CONSERVATIVE) + "\n";
	output += "ENDOGENOUS_OBSERVATION = " + std::to_string(CConfiguration::ENDOGENOUS_OBSERVATION) + "\n";
	output += "BOOLEAN_OBSERVATIONS = " + std::to_string(CConfiguration::BOOLEAN_OBSERVATIONS) + "\n";
	output += "PROBABILITY_THRESHOLD = " + std::to_string(CConfiguration::PROBABILITY_THRESHOLD) + "\n";
	...
	output += "NO_TURN_THRESHOLD = " + std::to_string(CConfiguration::NO_TURN_THRESHOLD.GetValue()) + "\n";
	output += "SOFT_TURN_THRESHOLD = " + std::to_string(CConfiguration::SOFT_TURN_THRESHOLD.GetValue()) + "\n";
	...
	output += "RANGE_THRESHOLD_AGGREGATION = " + std::to_string(CConfiguration::RANGE_THRESHOLD_AGGREGATION) + "\n";
	output += "INVALID_DISPLACEMENT_VECTOR = (" + std::to_string(CConfiguration::INVALID_DISPLACEMENT_VECTOR.GetX() ) +
		"," +std::to_string(CConfiguration::INVALID_DISPLACEMENT_VECTOR.GetY() ) + ")\n";
	output += "PROXIMITY_SENSOR_THRESHOLD_DISPERSION = " + std::to_string(CConfiguration::PROXIMITY_SENSOR_THRESHOLD_DISPERSION) + "\n";
	output += "GO_STRAIGHT_ANGLE_THRESHOLD_DISPERSION = " + std::to_string(CConfiguration::GO_STRAIGHT_ANGLE_THRESHOLD_DISPERSION.GetValue()) + "\n";
	output += "CHANGE_DIRECTION_PROBABILITY_RANDOMWALK = " + std::to_string(CConfiguration::CHANGE_DIRECTION_PROBABILITY_RANDOMWALK) + "\n";
	output += "NOT_BEACON_MESSAGE = " + std::to_string(CConfiguration::NOT_BEACON_MESSAGE) + "\n";
	output += "BEACON_MESSAGE = " + std::to_string(CConfiguration::BEACON_MESSAGE) + "\n";
	output += "BEACON_ID = " + std::to_string(CConfiguration::BEACON_ID) + "\n";
	output += "BEACON_BYTE_INDEX = " + std::to_string(CConfiguration::BEACON_BYTE_INDEX) + "\n";	
...
void computeNeighborsMatrix(CCI_RangeAndBearingSensor::TReadings& tRABReads, 
		NeighborAccumulator *matrix, NeighborAccumulator* accumulators,
		unsigned int TIME_WINDOW, unsigned int time, std::string id_str,
		bool beacon){
	...
	for(int i=0; i<N_ROBOTS; ++i){
		accumulators[i]={getRow(i)[time%TIME_WINDOW].neighborInnerRangeAccumulator,
...
void computeDisplacementsMatrix(CCI_RangeAndBearingSensor::TReadings& tRABReads, 
		const CCI_DifferentialSteeringSensor::SReading& tDifReads,
		float* lastWheelVel, DisplacementAccumulator* matrix, CVector2* displacements,
		unsigned int TIME_WINDOW, unsigned int time, std::string id_str)
{
	float deltaX = 0.0f;
	float deltaY = 0.0f;
	...
	for(size_t i=0; i<N_ROBOTS; ++i){
		for(int j=0; j<TIME_WINDOW; ++j){
			if(getRow(i)[j].validity){
				...
				deltaX = 0.0f;
				deltaY = 0.0f;
				computeOwnMovement(tDifReads.VelocityLeftWheel, tDifReads.VelocityRightWheel,
					lastWheelVel[0], lastWheelVel[1], getRow(i)[j].observerRotation.GetValue(),
					deltaOmega, deltaX, deltaY);
	...
	for(int i=0; i<tRABReads.size(); ++i){
		...
		if(getRow(id)[time%TIME_WINDOW].validity){
			 ...
			 */
			CVector2 newPosition = CVector2(tRABReads[i].Range, tRABReads[i].HorizontalBearing)
				.Rotate(getRow(id)[time%TIME_WINDOW].observerRotation) 
				+ getRow(id)[time%TIME_WINDOW].observerTranslation;

			displacements[id] = newPosition - getRow(id)[time%TIME_WINDOW].initialPosition;
		}else{
			displacements[id] = (CConfiguration::INVALID_DISPLACEMENT_VECTOR);
		}

		//Create a new DisplacementAccumulator.
		getRow(id)[time%TIME_WINDOW] = {CVector2(tRABReads[i].Range, tRABReads[i].HorizontalBearing),
	...
	if(CConfiguration::ENDOGENOUS_OBSERVATION){
    	...
    	displacements[id_int] = CVector2(getRow(id_int)[time%TIME_WINDOW].
     		observerTranslation.GetX(), 
	...
	// Create records for all the robots that haven't been observed in this control step.
	for(int i=0; i<N_ROBOTS; ++i){
		if(!observed[i]){
			displacements[i] = (CConfiguration::INVALID_DISPLACEMENT_VECTOR);

			getRow(i)[time%TIME_WINDOW] = {CVector2(), CVector2(), CRadians(), false};
		}
	}
}

void computeOwnMovement(float velLeftWheel, float velRightWheel, float lastVelLeftWheel,
	float lastVelRightWheel, float initialOmega, float& deltaOmega, float& deltaX, float& deltaY)
{
   ...
   if(lastVelLeftWheel != velLeftWheel || lastVelRightWheel != velRightWheel){
      ...
      if(lastVelLeftWheel != velLeftWheel){
         accelerationLeftWheel = (velLeftWheel - lastVelLeftWheel)/fabsf(velLeftWheel - lastVelLeftWheel)
            * (CConfiguration::ACCELERATION);
      }
      if(lastVelRightWheel!= velRightWheel){
         accelerationRightWheel = (velRightWheel - lastVelRightWheel)/fabsf(velRightWheel - lastVelRightWheel)
            * (CConfiguration::ACCELERATION);
      }

      while((velLeftWheelTemp != velLeftWheel || velRightWheelTemp !=velRightWheel) && timeStep > 0){
         timeStep -= CConfiguration::TIME_STEP_PHYSICS;
         deltaOmegaTemp = (-velLeftWheelTemp + velRightWheelTemp)*CConfiguration::TIME_STEP_PHYSICS/CConfiguration::INTERWHEEL_DISTANCE;

         /* initialOmega + deltaOmega is the rotation at the start of the physics simulation cycle, while deltaOmegaTemp
			 * is the rotation in the last cycle.
			 */
         deltaX += (velLeftWheelTemp + velRightWheelTemp)*CConfiguration::TIME_STEP_PHYSICS*cos(initialOmega+deltaOmega+deltaOmegaTemp/2)/2;
         deltaY += (velLeftWheelTemp + velRightWheelTemp)*CConfiguration::TIME_STEP_PHYSICS*sin(initialOmega+deltaOmega+deltaOmegaTemp/2)/2;

         deltaOmega += deltaOmegaTemp;

         if(velLeftWheelTemp > velLeftWheel){
         	velLeftWheelTemp = fmax(velLeftWheel, velLeftWheelTemp + accelerationLeftWheel * CConfiguration::TIME_STEP_PHYSICS);
         }else if(velLeftWheelTemp < velLeftWheel){
         	velLeftWheelTemp = fmin(velLeftWheel, velLeftWheelTemp + accelerationLeftWheel * CConfiguration::TIME_STEP_PHYSICS);
         }

         if(velRightWheelTemp > velRightWheel){
         	velRightWheelTemp = fmax(velRightWheel, velRightWheelTemp + accelerationRightWheel * CConfiguration::TIME_STEP_PHYSICS);
         }else if(velRightWheelTemp < velRightWheel){
            velRightWheelTemp = fmin(velRightWheel, velRightWheelTemp + accelerationRightWheel * CConfiguration::TIME_STEP_PHYSICS);
         }

      }
   }

   deltaOmegaTemp = (-velLeftWheel + velRightWheel)*timeStep/CConfiguration::INTERWHEEL_DISTANCE;
   deltaX += (velLeftWheel + velRightWheel)*timeStep*cos(initialOmega+deltaOmega+deltaOmegaTemp/2)/2;
   deltaY += (velLeftWheel + velRightWheel)*timeStep*sin(initialOmega+deltaOmega+deltaOmegaTemp/2)/2;
   deltaOmega += deltaOmegaTemp;
}

unsigned int stringReplace(std::string& string, char characterToFind, char characterToReplace){
